...
####################################### Edit your Setting #########################################
# Your ADB path
adb_path = "D:/software/platform-tools-latest-windows/platform-tools/adb.exe"

# Your GPT-4o API URL
API_url = "https://uiuiapi.com/v1/chat/completions"

# Your GPT-4o API Token
token = "sk-lrElVa1w02vzAl0pBf6a0364307542E6A64c51B964CdBe7b"
...
qwen_api = "sk-48a1f81c7a3f4d1d85fb07339f20c68f"

# Modal choice
choice = 3
# instruction = "Open the douyin app"

# Your instruction
if choice == 1:
    instruction = "打开百度地图导航到北京大学"

elif choice == 2:
    image_path = "./file/image/1.jpg"
    prompt = "I want to go to the place in this image. \
        What should I do with a mobile phone."


    chat = init_action_chat()
    chat = add_response("user", prompt, chat, image_path)
    instruction = inference_chat(chat, 'gpt-4o', API_url, token)

elif choice == 3:
    audio_path = "./file/audio/1.m4a"
    
    # 可将m4a文件转换为mp3文件
    mp3_path = "./file/audio/1.mp3"
    song = AudioSegment.from_file(audio_path)
    song.export(mp3_path, format="mp3")
    
    m = whisper.load_model('large')
    r=m.transcribe(audio_path, language='en', task='translate')
    instruction = r["text"]
    ...
    #         "prompt": "Translate it in English"
    #     }
    #
    #     # 发送POST请求
...
def get_perception_infos(adb_path, screenshot_file):
    ...
    if len(images) > 0:
        ...
        for i, j in zip(image_id, range(1, len(image_id) + 1)):
            if j in icon_map:
                if i < len(perception_infos):
                    perception_infos[i]['text'] = "icon: " + icon_map[j]
                else:
                    print(f"Index out of range: i={i}, perception_infos size={len(perception_infos)}")
            else:
                print(f"Index out of range: j={j}, icon_map size={len(icon_map)}")
